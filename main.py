...
# read the two images of the persons
for filename in os.listdir(image_dir):
    if filename.endswith(('.jpg', '.png', '.jpeg', 'webp')):
        ...
        person_names.append(split_list[0])

# face detection and saving
faces = face_detect(images, save_dir=save_dir)

# age prediction and reporting
ages = predict_age(faces)
print(f"\nFirst  person ({person_names[0]}) is: {ages[0]:.2f} years old")
print(f"Second person ({person_names[1]}) is: {ages[1]:.2f} years old")

# similarity check and reporting
similarity = check_similarity(faces)
print(f"\nSimilarity: {(100*similarity):.2f}%")
if similarity >= sim_threshold:
    print("Faces are MATCHED .. It is the same person")
else:
    print("Faces are NOT MATCHED .. They are two different persons")
